...
SWITCHES = [
  ...
  # ['-l', '--lint',            'pipe the compiled Uwsc through JavaScript Lint']
  ['-n', '--nodes',           'print out the parse tree that the parser produces']
  [      '--nodejs [ARGS]',   'pass options directly to the "node" binary']
  ['-o', '--output [DIR]',    'set the output directory for compiled Uwsc']
  ['-p', '--print',           'print out the compiled Uwsc']
...
fatalError = (message) ->
  ...
  console.log 'To see a list of all tasks/options, run "uwscscript --help"'
  process.exit 1
